...
{
    // Constant expression is an expression that can be evaluated during compilation time.to be a constant expression all the values must be known by the compiler at compilation time
    ...
    std::string result1 = (input_user <= 0) ? "False input program must be positive number!!" : "Correct you're inserting a positive number"; // this is also Runtime constant and the value cannot be known during compilation time because the result1 is not constant
    ...
    const std::string name{"Gerry"}; // <- the value of name "Gerry" will be known at compilation time
    const int sum2{5 + 5}; // <- the sum of 5 + 5 will be evaluated or calculated during Compilation Time not Runtime Constant

    constexpr int num1 = 5; // <- all variables that is declared with constexpr will be known by the compiler
    constexpr int num2 = 6; // <- all variables that is declared with constexpr will be known by the compiler
